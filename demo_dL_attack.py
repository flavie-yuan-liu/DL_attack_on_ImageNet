...
def main(args):
    ...
    # ----------------------------------------------------------------------
    lambda_grid_l1 = np.logspace(start=-4, stop=-2, num=3)
    lambda_grid_l2 = np.logspace(start=-3, stop=-1, num=3)
    ...
    attacks_hyper = {
        'ADiL': perf.get_atks(model.to(device), ADIL, 'lambda_l1', lambda_grid_l1, 'lambda_l2', lambda_grid_l2,
                              ...
                              batch_size=100, model_name=model_name, steps=80, attack_conditioned='atoms')
    }
    ...
    param_selection_file = 'dict_model_ImageNet/model_param_selection_atoms_supervised_budget.bin'
